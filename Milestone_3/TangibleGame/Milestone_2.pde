...
PVector getBoardAngles(PImage originalBis) {
  graph = new QuadGraph();
  anglesGetter = new TwoDThreeD(originalBis.width, originalBis.height);
  ...
  if(intersections.size() >= 4) {
    PVector angles = anglesGetter.get3DRotations(graph.sortCorners(intersections));
