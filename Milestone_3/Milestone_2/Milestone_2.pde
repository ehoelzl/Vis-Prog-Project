...
PImage original;
PImage scrollbars;
float lowerHue = 75; //96  75 100
float upperHue = 132.28; //140  
float lowerBrightness = 54.71;  //19  0 20
float upperBrightness = 192.84; // 171  232 150
float lowerSaturation = 88.71; // 58  99 130
float upperSaturation = 255; // 256
float minIntensity = 30; // 35
int nLines = 4;
...
void settings() {
  size(resizedWidth * 3, resizedHeight*2);
}
...
void setup() {
  ...
  cam =new Movie(this, "testvideo.mp4");//=new Capture(this, 640, 480);
...
//}
  ...
  vals = createGraphics(originalWidth*3,300);
  //noLoop();
}
...
void draw() {
  ...
  image(original, 0, 0, 640,480);
  nik = new TwoDThreeD(original.width, original.height);
   /* lowerHue = values.lowerhue.getPos()*255; //96  75 100
  upperHue = values.upperhue.getPos()*255; //129
   lowerBrightness =values.lowerbrightness.getPos()*255;;  //72
   upperBrightness = values.upperbrightness.getPos()*255; //177
     lowerSaturation = values.lowersaturation.getPos()*255; // 103
  upperSaturation = values.uppersaturation.getPos()*255; // 256
  print("lowerHue : " + lowerHue + " upperHue : " + upperHue + " lowerBrightness : " + lowerBrightness + " upper Brightness : " + upperBrightness +
    " lowerSaturation : " + lowerSaturation + " upperSatureation " + upperSaturation + "\n");*/
  List<PVector> intersections = drawIntersectionsAndQuads(lines(nLines, accumulator));
 
 if(intersections.size() >= 4) {
   
    PVector angles = nik.get3DRotations(graph.sortCorners(intersections));
    println(Math.toDegrees(angles.x) + " , " + Math.toDegrees(angles.y) + " , " + Math.toDegrees(angles.z) );
 }
  image(sobel, original.width, 0);
  image(hough, 2 * original.width, 0);
  image(vals, 0, originalHeight);
